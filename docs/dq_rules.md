...
## 8. Unicidade da Entidade Produto
Durante a criação da dimensão produtos (dim_products), na camada Gold (arquivo gold_dim_products.sql), foi constatado que 'product_id' não é estritamente único no dataset Silver. Isso ocorre devido a inconsistências na coluna 'product_name', fazendo com que o mesmo 'product_id' tenha valores diferentes em 'product_name', perdendo sua unicidade. A solução, na camada Gold, foi consolidar a entidade produto, mantendo um único registro por 'product_id', por meio de agrupamento e agregação determinística, utilizando a função MIN() para os atributos descritivos (product_name, category, sub_category). Isso garante o grão analítico de 1 produto único por linha na dimensão.